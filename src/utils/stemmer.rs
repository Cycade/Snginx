...
mod test {
    ...
    #[test]
    fn stemmer_test() {
        let mut test_set = HashMap::new();
        test_set.insert("fruitlessly", "fruitlessli");
        test_set.insert("possibilities", "possibl");
        test_set.insert("qualified", "qualifi");
        for term in test_set {
            assert_eq!(super::get(term.0).unwrap(), term.1);
        }
    }
}